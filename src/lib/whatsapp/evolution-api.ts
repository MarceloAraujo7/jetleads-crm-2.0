...
// A stuck WhatsApp session on the Evolution side (phone unlinked
// mid-session, Baileys reconnect loop, etc.) can leave its API
// hanging indefinitely instead of erroring — with no timeout, that
// froze the "Conectar número" modal's spinner forever. Every call
// below bounds the wait so the UI always gets an answer.
...
export async function sendMediaMessage(
  ...
  const { baseUrl, apiKey, instanceName, to, kind, link, caption, filename } =
    args;
